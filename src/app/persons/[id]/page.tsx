              ...
              <div className="mt-2.5 space-y-0.5 font-serif text-sm text-gray-600">
                {(person.birth_date || person.birth_place) && (
                  <div className="flex gap-1.5">
                    ...
                    <span>{[person.birth_date, person.birth_place].filter(Boolean).join(' · ')}</span>
                  </div>
                )}
                {(person.death_date || person.death_place) && (
                  <div className="flex gap-1.5">
                    <span className="text-gray-400 w-4">d.</span>
                    <span>{[person.death_date, person.death_place].filter(Boolean).join(' · ')}</span>
                  </div>
                )}
              </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {familyConnections.parents.map(p => (
                      ...
                      >
                        {personName(p)}
                        {p.birth_date && (
                          <span className="text-xs text-gray-400">b. {p.birth_date}</span>
                        )}
                      </Link>
                    ))}
                  </div>
            ...
            <div className="space-y-2">
              {todos.map(todo => (
                ...
                >
                  ...
                  />
                  <div className="flex-1 text-sm text-gray-800 font-serif">{todo.title}</div>
                  {todo.origin_module && (
