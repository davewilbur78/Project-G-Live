...
export async function GET(
  ...
  // NOTE: event_types join -- column may be 'name' or 'display_name'; Claude Code verify
  // NOTE: addresses columns -- verify street_address, city, state, latitude, longitude match schema
  const { data: timeline } = await supabase
    .from('timeline_events')
    .select(`
      id, event_date_display, event_date_sort, place_name, notes,
      source_id, address_id,
      event_types(id, name),
      sources(id, title, short_footnote_form, source_type, information_type, evidence_type),
      addresses(id, street_address, city, state, country, latitude, longitude)
    `)
    .eq('person_id', id)
  ...
  if (familyIds.length > 0) {
    const { data: familyData } = await supabase
      ...
      .select(`
        ...
        family_members(
          person_id, role,
          persons(id, given_name, surname, birth_date_display, death_date_display)
        )
      `)
      .in('id', familyIds);
  ...
  const { data: todos } = await supabase
    .from('todos')
    .select('id, description, priority, origin_module, created_at, status')
    .eq('person_id', id)
  ...
  const mapAddresses = (timeline || [])
    .filter((e: any) => e.addresses?.latitude && e.addresses?.longitude)
    .map((e: any) => ({
      ...e.addresses,
      event_type: e.event_types?.name || 'Event',
  ...
  return NextResponse.json({
    ...
    timeline: (timeline || []).map((e: any) => ({
      ...e,
      event_type_name: e.event_types?.name || e.event_type || 'Event',
    })),
    familyConnections,
    sources,
    todos: todos || [],
